...
def visa_connect(rm, device_address, timeout=5):
    ...
    t.start()
    t.join(timeout/1000)   # wait for timeout
...
class VisaApp:
    def __init__(self, root):
        ...
        tk.Label(root, text="Software: v8.0").pack(padx=10,anchor="ne")
...
class ManualModule:
    def __init__(self, root, rm):
        ...
        self.logs_csv_en = os.path.join(self.logs_dir, "CSV_EN")
        self.logs_csv_hu = os.path.join(self.logs_dir, "CSV_HU")
        self.logs_txt = os.path.join(self.logs_dir, "TXT")

        os.makedirs(self.logs_dir, exist_ok=True)
        os.makedirs(self.logs_csv_en, exist_ok=True)
        os.makedirs(self.logs_csv_hu, exist_ok=True)
        os.makedirs(self.logs_txt, exist_ok=True)
        ...
        self.visa_listbox.bind("<KeyRelease-space>", self.manual_measure_entry_safe)
        ...
        self.output_entry=ttk.Combobox(self.sub_frame, textvariable=self.output_format, values=[ "CSV [HU]", "CSV [EN]", "TXT"], width=9, justify="center")
        self.output_entry.grid(row=0, column=1,sticky="w")
        self.output_entry.bind("<space>", lambda e: "break")
        self.output_entry.bind("<KeyRelease-space>", self.manual_measure_entry_safe)
        self.output_entry.bind("<Delete>", lambda e: "break")
        self.output_entry.bind("<Delete>", self.delete_last_entry)
        self.output_format.trace_add("write", lambda *args: self.on_output_format_change())
        ...
        self.filename_entry.bind("<KeyRelease-space>", self.manual_measure_entry_safe)
        ...
        self.save_dir = tk.StringVar(value=self.logs_csv_hu)
        ...
        # ----------------- LIMIT selection ---------------------------------
        tk.Label(self.top1_frame, text="Min limit:").grid(row=5, column=2, padx=5, sticky='e')
        self.limit_min = tk.StringVar(value=f"{1e-9:.1g}") 
        self.limit_minentry = tk.Entry(self.top1_frame, textvariable=self.limit_min, width=31)
        self.limit_minentry.grid(row=5, column=3, padx=5, sticky='w', columnspan=4)
        self.limit_minentry.bind("<space>", lambda e: "break")
        self.limit_minentry.bind("<KeyRelease-space>", self.manual_measure_entry_safe)

        tk.Label(self.top1_frame, text="Max limit:").grid(row=6, column=2, padx=5, sticky='e')
        self.limit_max = tk.StringVar(value=f"{1e10:.1g}") 
        self.limit_maxentry = tk.Entry(self.top1_frame, textvariable=self.limit_max, width=31)
        self.limit_maxentry.grid(row=6, column=3, padx=5, sticky='w', columnspan=4)
        self.limit_maxentry.bind("<space>", lambda e: "break")
        self.limit_maxentry.bind("<KeyRelease-space>", self.manual_measure_entry_safe)
        ...
        self.top1_frame.bind("<KeyRelease-space>", self.manual_measure_entry_safe)
        ...
        self.console_text.bind("<KeyRelease-space>", self.manual_measure_entry_safe)
        self.console_text.bind("<Delete>", self.delete_last_entry)

        self.col_widths_console = None
    ...
    def on_output_format_change(self):
        ...
        if fmt == "CSV [HU]":
            save_folder = self.logs_csv_hu
        elif fmt == "CSV [EN]":
            save_folder = self.logs_csv_en
        else:
            save_folder = self.logs_txt

        self.save_dir.set(save_folder)

        parts = save_folder.replace("\\", "/").split("/")
        display_path = ".../" + "/".join(parts[-1:])
        ...
        if fmt in ["CSV [HU]", "CSV [EN]"]:
            path = os.path.join(save_folder, filename + ".csv")
            if not self.check_csv_language(path, fmt):

                # Delete console when filetype change
                self.console_text.config(state="normal")
                self.console_text.delete("1.0", tk.END)
                self.console_text.config(state="disabled")

    def check_csv_language(self, path, expected_format):
        ...
        # Choose delimiter by file format
        expected_delim = {"CSV [HU]": ";", "CSV [EN]": ",", "TXT": "\t"}.get(expected_format, ",")
        if file_delim != expected_delim:
            messagebox.showerror(
                "Error",
                f"CSV format mismatch!\nCurrent output format: {expected_format}\nDetected delimiter in file: '{file_delim}'"
            )
            return False
        return True
    def help2(self):
    ...
    def manual_measure(self, event=None):
        ...
        if os.path.isfile(output):
            with open(output, "r", encoding="utf-8") as f:
                ...
                lines = list(reader)
                if len(lines) > 1:  # when data exists
                    try:
    ...
    def insert_measurement_line(self, index, freq, z, p, new_file, label1, label2):
        ...
        if self.col_widths_console is None or new_file:
            ...
            self.console_text.insert(tk.END, header_line)
        ...
        LOW_LIMIT = float(self.limit_min.get())
        HIGH_LIMIT = float(self.limit_max.get())
        if z < LOW_LIMIT or z > HIGH_LIMIT:
            tag_to_use = "out_of_limit"
            #feedback_line = "⚠ Out of measurable range\n"
    ...
    def load_console_from_file(self):
        ...
        if os.path.isfile(path):
            # Check language and type
            if fmt in ["CSV [HU]", "CSV [EN]", "TXT"] and not self.check_csv_language(path, fmt):
                return
        ...
        try:
            ...
            with open(self.limit_file, "r", encoding="utf-8") as f:
                ...
                for row in reader:
                    ...
                    try:
                        ID = float(row[0])
                        # If HU format -> adjust delimiters
                        target = float(row[1].replace(",", "."))
                        plus   = float(row[2].replace(",", "."))
                        minus  = float(row[3].replace(",", "."))
                        self.limits.append((ID, target, plus, minus))
                    except ValueError:
                        continue

            messagebox.showinfo("Info", f"Limit file loaded: {len(self.limits)} rows")
    ...
    def delete_last_entry(self, event=None):
        ...
        # Warning on no measurement data
        if len(rows) <= 1:
        ...
        if self.last_measure_extra_line > 0:
            self.console_text.delete("end-3l", "end-1l")   # delete 2 rows
            self.last_measure_extra_line-=1
        else:
            self.console_text.delete("end-2l", "end-1l")   # delete 1 row
...
class SweepModule: 
    def __init__(self, root, rm):
        ...
        self.logs_dir = os.path.join(self.base_dir, "logs")
        self.logs_csv_en = os.path.join(self.base_dir, "logs", "CSV_EN")
        os.makedirs(self.logs_csv_en, exist_ok=True)
        #self.logs_csv_en = os.path.join(self.logs_dir, "CSV_EN")
        self.logs_csv_hu = os.path.join(self.logs_dir, "CSV_HU")
        self.logs_txt = os.path.join(self.logs_dir, "TXT")
    ...
    def start_measurement(self):
        ...
        self.output_csv = os.path.join(self.logs_csv_en, self.filename.get() + ".csv")
