...
class VisaApp:
    def __init__(self, root):
        ...
        tk.Label(root, text="Software: v5.0").pack(padx=10,anchor="ne")
...
class ManualModule:
    def __init__(self, root, rm):
        ...
        self.limit_entry = tk.Entry(self.top1_frame, textvariable=self.limit_display, width=31)
        ...
        try:
            with open(output, "a", newline="", encoding="utf-8") as f:
                ...
                if not lines:  # if new file write header
                    ...
                    new_file=True
                writer.writerow([current_index, freq, z, p])
        except Exception as e:
    ...
    def insert_measurement_line(self, index, freq, z, p, new_file, label1, label2):
        ...
        if self.col_widths_console is None or new_file:
            ...
            for i, lbl in enumerate(header_labels):
                header_line += f"{lbl:^{self.col_widths_console[i]}} "
            self.console_text.insert(tk.END, header_line.rstrip() + "\n")

        # Sor formázása
        line = f"{index:^{self.col_widths_console[0]}} {freq:^{self.col_widths_console[1]}.6g} {z:^{self.col_widths_console[2]}.6g} {p:^{self.col_widths_console[3]}.6g}\n"
    ...
    def load_console_from_file(self):
        ...
        save_folder = self.save_dir.get() if self.save_dir.get() else self.logs_dir
        path = os.path.join(save_folder, filename + ".csv") if self.output_format.get()=="csv" else os.path.join(save_folder, filename + ".txt")
        delimiter = "," if self.output_format.get()=="csv" else "\t"
            ...
            if rows:
                ...
                for row in rows:
                    ...
                    for i, cell in enumerate(row):
                        line += f"{cell:^{self.col_widths_console[i]}} "
                    self.console_text.insert(tk.END, line.rstrip() + "\n")
    ...
    def delete_last_entry(self, event=None):
        ...
        # utolsó sor törlése
        self.console_text.delete("end-2l", "end-1l")
