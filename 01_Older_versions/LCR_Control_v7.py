...
class ManualModule:
    ...
    def on_output_format_change(self):
        ...
        if fmt in ["CSV [HU]", "CSV [EN]"]:
            ...
            if not self.check_csv_language(path, fmt):
                # Ha nem egyezik, töröljük a konzolt
    ...
    def check_csv_language(self, path, expected_format):
        ...
        # Meghatározzuk a várt delimiter-t a formátum alapján
        ...
        try:
            ...
            with open(self.limit_file, "r", encoding="utf-8") as f:
                ...
                for row in reader:
                    ...
                    try:
                        ID = float(row[0])
                        # Ha HU formátum, konvertáljuk a ','-t '.'-ra floathoz
    ...
    def check_limits(self):
        ...
        elif self.output_format.get()=="CSV [EN]":
            path = os.path.join(save_folder, self.filename.get() + ".csv")
            delimiter = ","
        else:
