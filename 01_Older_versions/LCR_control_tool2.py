...
import time   #Timing
import numpy as np #Calculation module
import os          #Handle files
from tkinter import ttk #Combobox widget
import matplotlib       #Plot module
matplotlib.use("TkAgg") #GUI compatible backend
import matplotlib.pyplot as plt
import sys              #Check runtime environment
import skrf as rf       #Module for handling S-parameters
...
class VisaApp:
    def __init__(self, root):
        ...
        self.sweep_module = SweepModule(self.sweep_tab)
        self.plot_module = PlotModule(self.plot_tab)
        self.manual_module = ManualModule(self.manual_tab)
...
class PlotModule: # Plot module
    def __init__(self, root):
        ...
        ttk.Combobox(self.frame, textvariable=self.xscale, values=["lin", "log"], width=9, justify="center").grid(row=3, column=2,sticky="w")

        tk.Label(self.frame, text="      Y axis type:").grid(row=3, column=3, sticky="e", pady=10)
        self.yscale = tk.StringVar(value="lin")
        ttk.Combobox(self.frame, textvariable=self.yscale, values=["lin", "log"], width=9, justify="center").grid(row=3, column=4,sticky="w")
